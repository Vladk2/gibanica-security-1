...
import {getNumberOfLogsInsertedPerDay, getNumberOfLogsInsertedPerHost, getNumberOfLogsInserted} from "../util/LogsApi";
    ...
    getNumberOfLogsInserted().then(res_sys => {
      getNumberOfLogsInsertedPerDay(30).then(res_days => {
        getNumberOfLogsInsertedPerHost().then(res_host => {
          this.setState({dataSystem: res_sys, dataDays: res_days, dataHost: res_host});
        });
      });
    });
  }
  ...
  render() {
    ...
    return (
      <div
        ...
        background: "linear-gradient(to top, #CBCBCB, #CBCBCB, #E7E7E7, #F5F5F5, white, white, white)"
      }}>
