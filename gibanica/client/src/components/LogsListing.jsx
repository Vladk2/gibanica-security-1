...
export default class LogsListing extends React.Component {
  ...
  search = () => {
    // will be changed
    fetch(`https://localhost:3000/logs?` + `filterBy=${this.state.filterMenu.value}` + `&searchBy=${this.searchBy}`, {
        headers: {
  ...
  fetchLogs = page => {
    getLogsPerPage(this.checkPage(page)).then(res => {
      this.setState({logs: res.data.data, pagesCount: res.count, currentPage: res.page});
    });
  };
